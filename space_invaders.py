...
def run_single_agent_episode(
    ...
    def apply_local_reward(delta, reward_key):
        nonlocal fitness_delta
        fitness_delta += delta
    ...
    while player.health > 0 and lives_local > 0 and frame_count < max_frames:
        frame_count += 1
        ...
        if len(enemies) == 0:
            if boss_active:
                ...
                if reward_values:
                    apply_local_reward(wave_clear_reward, "wave_clear_reward_total")
                spawn_boss_wave()
            else:
                spawn_regular_wave()

        if reward_values:
            apply_local_reward(survival_reward, "survival_reward_total")
        ...
        if output[2] > 0.5:
            ...
            if len(player.lasers) > shots_before:
                ...
                if reward_values:
                    apply_local_reward(-shot_penalty, "shot_penalty_total")
                ...
                if laser.collision(enemy):
                    ...
                    if enemy.health <= 0 and enemy in enemies:
                        ...
                        if reward_values:
                            ...
                            apply_local_reward(reward_delta, "kill_reward_total")
                    break
        ...
        for enemy in enemies[:]:
            ...
            for laser in enemy.lasers[:]:
                ...
                if laser.collision(player):
                    ...
                    if reward_values:
                        apply_local_reward(-laser_hit_penalty, "death_penalty_total")
                    if laser in enemy.lasers:
                        enemy.lasers.remove(laser)

            if enemy.y + enemy.get_height() > HEIGHT:
                lives_loss = 2 if isinstance(enemy, Boss) else 1
                lives_local -= lives_loss
                event_totals["enemy_escapes"] += 1
                if reward_values:
                    apply_local_reward(-enemy_escape_penalty * lives_loss, "enemy_escape_penalty_total")
                enemies.remove(enemy)
                continue
        ...
        if player.health <= 0:
            ...
            if reward_values:
                apply_local_reward(-death_penalty, "death_penalty_total")
            break

        if lives_local <= 0:
            event_totals["level_failures"] += 1
            if reward_values:
                apply_local_reward(-level_fail_penalty, "level_fail_penalty_total")
            break

        if draw_episode and win is not None:
            lives = lives_local
            active_boss = next((enemy for enemy in enemies if isinstance(enemy, Boss)), None)
            draw_window(win, enemies, [player], gen_label, level, active_boss)

    return {
        "fitness_delta": fitness_delta,
        "frames": frame_count,
        "lives_remaining": lives_local,
        "player_alive": int(player.health > 0 and lives_local > 0),
...
def eval_genomes(genomes, config):
    ...
    lives = int(round(average_lives_sum / population_size))
...
def run(config_file, best_genome_path=BEST_GENOME_PATH, generations=50, reward_profile_name=None, metrics_path=None):
    ...
    global gen, ACTIVE_TRAINING_METRICS_PATH
