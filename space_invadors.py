# In this version of Space Invadors the player can only move left and right.
...
# i.e. surpases the space ship of the player.
...
def eval_genomes(genomes, config):
    ...
    while run and len(players) > 0:
        ...
        for player in players:  # give each bird a fitness of 0.1 for each frame it stays alive
            ...
            if output[0] > 0.5 and player.x + player.PLAYER_VEL + player.get_width() < WIDTH:  # we use a tanh activation function so result will be between -1 and 1. if over 0.5 go right
                player.move_right()
        ...
        for enemy in enemies:
            ...
            for player in players:
                ...
                # each player that colidet with the current enemy will be remove
                if collide(enemy, player):
                    ge[players.index(player)].fitness -= 1
                    nets.pop(players.index(player))
                    ge.pop(players.index(player))
                    players.pop(players.index(player))
                    # the enemy has to be removed after every player has been checked for cullision
                    #try:
...
def run(config_file):
    # runs the NEAT algorithm to train a neural network to play space invadors.
